...
- Split mode does not register the legacy `/chat` slash command; Noona's natural mention chat still reuses that saved
  command gate for role checks. In single-bot fallback mode, legacy `/chat` uses Noona's same one-at-a-time AI queue
  and edits its ephemeral `Thinking...` response when the answer is ready.
...
settings/env/auth/sync diagnostics, Appa mention/review/correction state, default-avatar sync results, the most recent
Appa mention-gate rejection, and the last meaningful Discord runtime error when a bot disconnects. A settings refresh
can also retry the split runtime when Appa was waiting on brokered settings or previously fell back to Noona.
...
by the slash command, sends a requester-mentioned `Thinking...` reply immediately, asks Sage, and edits that same
message with the final response. If another request is already running for that bot, Portal first replies with the
number of request(s) ahead, then edits the placeholder to `Thinking...` when the job starts. It ignores bots, wrong
guilds, empty prompts, disallowed channels, and ordinary unmentioned chatter. Mention chat returns `true` to the guild
message pipeline when it handles a message, so trivia does not also process the same message as a guess. Portal allows
only the requesting Discord user mention in these AI replies and keeps long response chunking by editing the first
chunk, then sending overflow chunks as follow-ups.
...
the background through Appa's own AI queue, so Appa admin mention chats and review work stay one-at-a-time. Appa posts
a same-thread correction only for serious issues such as leaked secrets, unsafe action claims, admin-boundary mistakes,
or clearly wrong operational facts. Sage stores redacted review and delivery events in Vault; it never stores full
Discord transcripts.
...
posts one compact digest channel message with up to ten Moon read or title links plus a `+N more` count, and
acknowledges Sage with the digest metadata only after the send succeeds.
...
Portal does not call GitHub or Oracle directly for update posts. The Discord message keeps Noona's polished summary in
one embed description, with build, compare, and admin links in fields, and intentionally leaves raw commit rows behind
those links.

...
export const createPortalApp = async ({
  ...
  app.post("/api/notifications/update/test", async (req, res) => {
    try {
      await runtime.refreshSettings().catch(() => {});
      res.json(await runtime.sendUpdateNotificationTest(req.body || {}));
    } catch (error) {
      logger.warn("Portal update notification test failed.", {error});
      res.status(409).json({
        error: error instanceof Error ? error.message : String(error)
      });
    }
